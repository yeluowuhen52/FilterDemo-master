 ...
 */
package com.chs.filterdemo.util;
...
public class HanziToPinyinUtil {
    ...
    public static HanziToPinyinUtil getInstance() {
        synchronized (HanziToPinyinUtil.class) {
            ...
            Log.w(TAG, "There is no Chinese collator, HanziToPinyinUtil is disabled");
            sInstance = new HanziToPinyinUtil(false);
            return sInstance;
        }
    }
    ...
    public static String getPinYin(String input) {
        ArrayList<Token> tokens = HanziToPinyinUtil.getInstance().get(input);
        StringBuilder sb = new StringBuilder();
        if (tokens != null && tokens.size() > 0) {
            for (HanziToPinyinUtil.Token token : tokens) {
                if (token.type == HanziToPinyinUtil.Token.PINYIN) {
