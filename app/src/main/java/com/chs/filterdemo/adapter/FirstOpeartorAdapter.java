...
public class FirstOpeartorAdapter extends KJAdapter<Contact> implements SectionIndexer {
    ...
    public FirstOpeartorAdapter(AbsListView view, ArrayList<Contact> mDatas) {
        super(view, mDatas, R.layout.item_list_opeartor_person);
        datas = mDatas;
        if (datas == null) {
            datas = new ArrayList<>();
        }
    ...
    public void convert(AdapterHolder holder, Contact item, boolean isScrolling, int position) {
        ...
        //如果是第0个那么一定显示#号
        if (position == 0) {
            Contact prevData = datas.get(0);
            tvLetter.setVisibility(View.VISIBLE);
            tvLetter.setText(prevData.getFirstChar()+"");
